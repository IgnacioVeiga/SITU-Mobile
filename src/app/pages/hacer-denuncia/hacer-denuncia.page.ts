...
export class HacerDenunciaPage {
  ...
  generarId(): number {
    ...
    do {
      // Se debe verificar que ese id no esté en uso
      const algunaFoto = this.fotos.find((a) => a.id === numero);

      // Si existe alguna foto con ese id, entonces entro al condicional
      if (algunaFoto) {
        esRepetido = true;

        // Se genera otro numero aleatorio
        numero = Math.round(Math.floor(Math.random() * 200000) + 100000);
      }
    } while (esRepetido);
    return numero;
  }
